"""
Backbone: DINOv2
Click encoder: Simple ViT
Upsampler: Identity
Click Features Injection: after backbone (late injection)
"""
...
def define_modules_cfg(cfg: DictConfig) -> edict:
    ...
    """
    # unique model name
    MODEL_NAME = "sbd_dinov2_noup_convhead_simplevit_lateinject_224"
    ...
    backbone_cfg = dict(
        type="dinov2",
        params=dict(
            feats_injection_mode="after_backbone",
        ),
    )
    ...
    embed_coords_cfg = dict(
        type="simple_vit",
        params=dict(
            img_size=list(cfg.training_params.crop_size),
            patch_size=(14, 14),
            embed_dim=384,
            depth=6,
            heads=8,
    ...
    upsampler_cfg = dict(type="identity", params=dict())
    neck_cfg = None
