...
def define_modules_cfg(cfg: DictConfig) -> edict:
    ...
    """
    # unique model name
    MODEL_NAME = "sbd_vit_noup_convhead_patchembed_earlyinject_224"
    ...
    backbone_cfg = dict(
        ...
        params=dict(
            ...
            patch_size=16,
            feat_type="key",
            feats_injection_mode="before_backbone",  # for late injection, set to "after_backbone"
        ),
    )
    ...
    embed_coords_cfg = dict(
        type="patchEmbed",
        params=dict(
            img_size=cfg.training_params.crop_size, patch_size=(16, 16), embed_dim=384
        ),
    )
