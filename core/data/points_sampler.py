"""Class for generating synthetic clicks that mimic realistic human interactions. Used during training phase."""
...
class BasePointSampler:
    ...
    def selected_mask(self, mask: np.ndarray) -> None:
...
class MultiPointSampler(BasePointSampler):
    def __init__(
        self,
        max_num_points: int,
        prob_gamma: float = 0.7,
        expand_ratio: float = 0.1,
        positive_erode_prob: float = 0.9,
        positive_erode_iters: int = 3,
        negative_bg_prob: float = 0.1,
        negative_other_prob: float = 0.4,
        negative_border_prob: float = 0.5,
        merge_objects_prob: float = 0.0,
        max_num_merged_objects: int = 2,
        use_hierarchy: bool = False,
        soft_targets: bool = False,
        first_click_center: bool = False,
        only_one_first_click: bool = False,
        sfc_inner_k: float = 1.7,
        sfc_full_inner_prob: float = 0.0,
    ) -> None:
        super().__init__()
    ...
    def sample_object(self, sample: DSample) -> None:
        if len(sample) == 0:
    ...
    def _sample_mask(
        ...
        for obj_id in random_ids:
            obj_gt_mask, obj_pos_segments, obj_neg_segments = (
    ...
    def _sample_from_masks_layer(
        self, obj_id: int, sample: DSample
        ...
        def _select_node(node_id: int) -> int:
    ...
    def sample_points(self) -> List[np.ndarray]:
        assert self._selected_mask is not None
    ...
    def _multi_mask_sample_points(
        self, selected_masks: List, is_negative: bool, with_first_click: bool = False
    ) -> List[np.ndarray]:
    ...
    def _positive_erode(self, mask: np.ndarray) -> np.ndarray:
    ...
    def _get_border_mask(self, mask: np.ndarray) -> np.ndarray:
...
def generate_probs(max_num_points: int, gamma: float) -> np.ndarray:
    probs = []
    last_value = 1
...
def get_point_candidates(
    obj_mask: np.ndarray, k: float = 1.7, full_prob: float = 0.0
) -> np.ndarray:
