"""Additional predictors implementing the Backpropagation Refinement Scheme (BRS).
These predictors were not part of the original evaluation process."""
...
from .brs_optimizers import BaseOptimizer
...
class BRSBasePredictor(BasePredictor):
    def __init__(
        ...
        model: iSegBaseModel,
        device: str,
        opt_functor: BaseOptimizer,
        optimize_after_n_clicks: int = 1,
        **kwargs
    ) -> None:
    ...
    def set_input_image(self, image: Union[torch.Tensor, np.ndarray]) -> None:
        super().set_input_image(image)
        self.opt_data = None
        self.input_data = None

    def _get_clicks_maps_nd(
        self, clicks_lists: List[List[Click]], image_shape: Tuple, radius: int = 1
...
class FeatureBRSPredictor(BRSBasePredictor):
    def __init__(
        ...
        device: str,
        opt_functor: BaseOptimizer,
        insertion_mode: str = "after_deeplab",
        **kwargs
    ) -> None:
    ...
    def _get_prediction(
        self,
        image_nd: torch.Tensor,
        clicks_lists: List[List[Click]],
        is_image_changed: bool,
    ) -> torch.Tensor:
    ...
    def _get_head_input(
        self, image_nd: torch.Tensor, points: torch.Tensor
    ) -> torch.Tensor:
        with torch.no_grad():
...
class InputBRSPredictor(BRSBasePredictor):
    ...
    def _get_prediction(
        self, image_nd: torch.Tensor, clicks_lists: torch.Tensor, is_image_changed: bool
    ) -> torch.Tensor:
        points_nd = self.get_points_nd(clicks_lists)
        pos_mask, neg_mask = self._get_clicks_maps_nd(clicks_lists, image_nd.shape[2:])
        num_clicks = len(clicks_lists[0])
        ...
        def get_prediction_logits(opt_bias: torch.Tensor) -> torch.Tensor:
