"""Trainer for interactive segmentation models."""
...
from core.training.metrics import TrainMetric
from core.utils.distributed import get_dp_wrapper, get_sampler, reduce_loss_dict
from core.utils.log import SummaryWriterAvg, TqdmToLogger, logger
from core.utils.misc import get_device, save_checkpoint, seed_worker
from core.utils.serialization import get_config_repr
from core.utils.viz import draw_points, draw_probmap

from .optimizer import get_optimizer, get_optimizer_with_layerwise_decay


class iSegTrainer(object):
    def __init__(
        self,
        model: iSegBaseModel,
        cfg: DictConfig,
        loss_cfg: Dict,
        trainset: iSegBaseDataset,
        valset: iSegBaseDataset,
        optimizer: str = "adam",
        optimizer_params: Dict = None,
        layerwise_decay: bool = False,
        image_dump_interval: int = 200,
        checkpoint_interval: int = 10,
        tb_dump_period: int = 25,
        max_interactive_points: int = 0,
        lr_scheduler: LRScheduler = None,
        metrics: List[TrainMetric] = None,
        additional_val_metrics: List[TrainMetric] = None,
        net_inputs: Tuple[str, str] = ("images", "points"),
        max_num_next_clicks: int = 0,
        click_models=None,
        prev_mask_drop_prob: float = 0.0,
        seed: int = -1,
    ) -> None:
        self.cfg = cfg
        ...
        if cfg.training.distributed:
            cfg.dataloader.batch_size //= cfg.training.ngpus
            cfg.dataloader.val_batch_size //= cfg.training.ngpus
        ...
        logger.info(
            f"Dataset of {self.trainset.get_samples_number()} samples was loaded for training."
        )
        logger.info(
            f"Dataset of {self.valset.get_samples_number()} samples was loaded for validation."
        )
        ...
        if cfg.training.multi_gpu:
            model = get_dp_wrapper(cfg.training.distributed)(
                model,
                device_ids=cfg.training.gpu_ids,
                output_device=cfg.training.gpu_ids[0],
            )
        ...
        if lr_scheduler is not None:
            ...
            if cfg.training.start_epoch > 0:
                for _ in range(cfg.training.start_epoch):
    ...
    def run(
        self, num_epochs: int, start_epoch: bool = False, validation: bool = True
    ) -> None:
        if start_epoch is None:
            start_epoch = self.cfg.training.start_epoch
    ...
    def training(self, epoch: int) -> None:
        ...
        for i, batch_data in enumerate(tbar):
            ...
            losses_logging["overall"] = (
                loss  # sum of 'instance_loss' and 'instance_aux_loss'
            )
            ...
            if self.is_master:
                # add: log epoch to tensorboard
                self.sw.add_scalar(tag=f"epoch", value=epoch, global_step=global_step)
        ...
        if self.is_master:
            ...
            save_checkpoint(
                ...
                epoch=None,
                multi_gpu=self.cfg.training.multi_gpu,
            )
    ...
    def batch_forward(
        self, batch_data: Dict, validation: bool = False
        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                num_iters = random.randint(
                    0, self.max_num_next_clicks
                )  # NOTE: randomly select number of iterations
                    ...
                    for m in metrics:
                        m.update(
                            *(
                            ...
                            ),  #  e.g., m.pred_outputs = ['instances']
                            *(batch_data[x] for x in m.gt_outputs),
                        )  #  e.g., m.gt_outputs = ['instances']
    ...
    def save_visualization(
        self, splitted_batch_data: Dict, outputs: Dict, global_step: int, prefix: str
    ) -> None:
    ...
    def _load_weights(self, net: iSegBaseModel) -> iSegBaseModel:
        if self.cfg.training.weights is not None:
            if os.path.isfile(self.cfg.training.weights):
                load_weights(net, self.cfg.training.weights)
                self.cfg.training.weights = None
            else:
                raise RuntimeError(
                    f"=> no checkpoint found at '{self.cfg.training.weights}'"
                )
        elif self.cfg.training.resume_exp is not None:
            checkpoints = list(
                self.cfg.CHECKPOINTS_PATH.glob(
                    f"{self.cfg.training.resume_prefix}*.pth"
                )
            )
    ...
    def is_master(self) -> bool:
        return self.cfg.training.local_rank == 0
...
def load_weights(model: iSegBaseModel, path_to_weights: str) -> None:
    ...
    logger.info(f"Loaded weights from {path_to_weights} with msg: {msg}")
