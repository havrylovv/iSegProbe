...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.current_scale = (
            1.0  # scale for the canvas image zoom, public for outer classes
        )
        ...
        self.canvas.bind(
            "<Configure>", lambda event: self.__size_changed()
        )  # canvas is resized
        self.canvas.bind(
            "<Button-1>", self.__left_mouse_button
        )  # remember canvas position
        self.canvas.bind(
            "<ButtonPress-3>", self.__right_mouse_button_pressed
        )  # remember canvas position
        self.canvas.bind(
            "<ButtonPress-2>", self.__right_mouse_button_pressed
        )  # remember canvas position (MacOS)
        self.canvas.bind(
            "<ButtonRelease-3>", self.__right_mouse_button_released
        )  # remember canvas position
        self.canvas.bind(
            "<ButtonRelease-2>", self.__right_mouse_button_released
        )  # remember canvas position (MacOS)
        self.canvas.bind(
            "<B3-Motion>", self.__right_mouse_button_motion
        )  # move canvas to the new position
        self.canvas.bind(
            "<B2-Motion>", self.__right_mouse_button_motion
        )  # move canvas to the new position
        self.canvas.bind(
    ...
    def reload_image(self, image, reset_canvas=True):
        ...
        if reset_canvas:
            ...
            scale = min(
                self.canvas.winfo_width() / self.imwidth,
    ...
    def __show_image(self):
        ...
        if (
            ...
            crop_w, crop_h = math.ceil(sx2 - sx1 + 2 * border_width), math.ceil(
                sy2 - sy1 + 2 * border_width
            )
            crop_w = min(crop_w, self.__original_image.width - crop_x)
            crop_h = min(crop_h, self.__original_image.height - crop_y)

            __current_image = self.__original_image.crop(
    ...
    def _reset_canvas_offset(self):
        ...
        self.canvas.scan_dragto(
            int(self.canvas.canvasx(0)), int(self.canvas.canvasy(0)), gain=1
        )
    ...
    def _change_canvas_scale(self, relative_scale, x=0, y=0):
        ...
        self.canvas.scale(
            "all", x, y, relative_scale, relative_scale
        )  # rescale all objects
    ...
    def __size_changed(self):
        new_scale_w = self.canvas.winfo_width() / (
            self.current_scale * self.__original_image.width
        )
        new_scale_h = self.canvas.winfo_height() / (
            self.current_scale * self.__original_image.height
        )
    ...
    def __wheel(self, event):
        ...
        if (
            event.num == 5 or event.delta == -120 or event.delta == 1
        ):  # scroll down, zoom out, smaller
            scale /= self.__delta
        if (
            event.num == 4 or event.delta == 120 or event.delta == -1
        ):  # scroll up, zoom in, bigger
            scale *= self.__delta
    ...
    def __right_mouse_button_released(self, event):
        ...
        move_delta = math.sqrt(
            (event.x - self._last_rb_click_event.x) ** 2
