...
from core.model.ops import BatchImageNormalize, DistMaps, ScaleLayer


class iSegBaseModel(nn.Module):
    def __init__(
        self,
        with_aux_output: bool = False,
        norm_radius: int = 5,
        use_disks: bool = False,
        cpu_dist_maps: bool = False,
        use_rgb_conv: bool = False,
        use_leaky_relu: bool = False,  # use_rgb_conv and use_leaky_relu are used only for RITM
        with_prev_mask: bool = False,
        norm_mean_std: Tuple[List, List] = (
            [0.485, 0.456, 0.406],
            [0.229, 0.224, 0.225],
        ),
    ) -> None:
    ...
    def forward(self, image: torch.Tensor, points: torch.Tensor) -> Dict:
        image, prev_mask = self.prepare_input(
            image
        )  # originally, image may have 4 channels, where the 4th channel is the prev_mask
    ...
    def prepare_input(self, image: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        prev_mask = None
        if self.with_prev_mask:
            prev_mask = image[:, 3:, :, :]
            image = image[:, :3, :, :]
    ...
    def get_coord_features(
        self, image: torch.Tensor, prev_mask: torch.Tensor, points: torch.Tensor
    ) -> torch.Tensor:
    ...
    def get_state_dict_to_save(self) -> Dict:
        """Defines which model components will be saved in the checkpoint. Called by the trainer.
        The default implementation saves all model parameters.
        Override this method to customize the saving behavior."""
