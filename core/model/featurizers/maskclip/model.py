...
class VisionTransformer(nn.Module):
    ...
    def forward_without_patch_embed(
        self,
        x: torch.Tensor,
        orig_image_hw: Tuple[int, int],
        patch_output: bool = False,
    ):
        """
        Forward pass through the model without patch embedding layer.

        Args:
            x (torch.Tensor): Input tensor of shape [batch_size, channels, height, width].
            orig_image_hw (Tuple[int, int]): Original image height and width.
            patch_output (bool, optional): If True, returns the patch tokens instead of the class token. Defaults to False.
        Returns:
            torch.Tensor: Output tensor after processing through the model. If `patch_output` is True, returns the patch tokens; otherwise, returns the class token.
        """
...
class CLIP(nn.Module):
    ...
    def encode_projected_patches(
        self, patches, orig_image_hw: Tuple[int, int]
    ) -> torch.Tensor:
        """
        Encodes projected image patches using the VisionTransformer model.
        Projected patches are those that have been passed through the projection / embidding layer.
        (e.g., output of self.conv1 in VisionTransformer)

        Args:
            patches (torch.Tensor): The image patches to encode.
            orig_image_hw (Tuple[int, int]): The original height and width of the image.

        Returns:
            torch.Tensor: The encoded patches.

        Raises:
            ValueError: If the model is not an instance of VisionTransformer.
        """

        if not isinstance(self.visual, VisionTransformer):
            raise ValueError(
                "This method is only supported for VisionTransformer models."
            )

        return self.visual.forward_without_patch_embed(
            patches.type(self.dtype), orig_image_hw=orig_image_hw, patch_output=True
        )
