"""DINO Featurizer, adapter for injection of additional features. Used also as ViT featurizer."""
...
from core.utils.log import logger as isegm_logger
...
class DINOFeaturizer(nn.Module):
    """DINO Featurizer, adapter for injection of additional features. Used also as ViT featurizer in our experiments.
    Args:
        arch (str): Architecture name. We used `vit_small_patch16_224` for VIT experiments.
        patch_size (int): Patch size.
        feat_type (str): Feature type, either 'key' or 'token'.
    ...
    def __init__(
        ...
        patch_size: int,
        feat_type: str = "key",
        feats_injection_mode: str = "no_injection",
    ) -> None:
        super().__init__()
        self.arch = arch
        self.patch_size = patch_size
        ...
        assert self.feats_injection_mode in [
            "before_backbone",
            "after_backbone",
        ], f"Unknown feats_injection_mode: {self.feats_injection_mode}"

        isegm_logger.info(f"Loaded checkpoint for DINO: {arch}")
        isegm_logger.info(f"Feats Injection Mode: {feats_injection_mode}")
    ...
    def forward(
        ...
        for i, blk in enumerate(self.model.blocks):
            ...
            if len(self.model.blocks) - i <= n:
                feat.append(self.model.norm(x))
                qkvs.append(qkv)
                attns.append(attn)

        feat, attn, qkv = feat[0], attns[0], qkvs[0]
        ...
        if self.feat_type == "token":
            ...
            if (
                ...
                assert (
                    feat.shape == additional_features.shape
                ), f"feat.shape: {feat.shape}, additional_features.shape: {additional_features.shape}"
                feat += additional_features.to(feat.dtype)
            image_feat = feat.reshape(feat.shape[0], feat_h, feat_w, -1).permute(
                0, 3, 1, 2
            )
            ...
            if (
                ...
                assert (
                    desc.shape == additional_features.shape
                ), f"desc.shape: {desc.shape}, additional_features.shape: {additional_features.shape}"
                desc += additional_features.to(desc.dtype)
